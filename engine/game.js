...
const Game = {
    ...
    _getInnateTalentGrowthHint(talent) {
        ...
        // 传说级：出生即巅峰，天赋完满
        if (rarity === 'legendary' && evoCount === 0) {
            const hints = {
        ...
        // 旧版天赋（无evolutions）：暗示有进化潜力，待觉醒
        // 史诗级：进化空间大
        if (rarity === 'epic') {
            const hints = {
                '元素亲和': '与元素的羁绊才刚刚开始，深处的共鸣尚待唤醒',
                '修炼天才': '悟性的种子已埋下，假以时日必将枝繁叶茂',
                '强健体魄': '肉身的潜能远未穷尽，千锤百炼后方见真章',
                '精神强大': '精神力的深渊深不可测，探索才刚刚开始',
                '致命一击': '对破绽的敏锐可以更加锐利，一击必杀的境界尚在前方',
                '风之迅捷': '速度的极限远未触及，御风而行的境界尚待领悟',
                '吸血本能': '嗜血的本能可以进化得更加精妙，生命汲取的奥秘无穷'
            };
            return hints[name] || '天赋潜力深厚，成长空间广阔';
        }
        
        // 稀有级：有一定进化空间
        if (rarity === 'rare') {
            const hints = {
                '快速施法': '施法的流畅度仍可精进，瞬发的境界尚在前方',
                '钢铁意志': '心如磐石，但磐石亦可打磨成器',
                '幸运星': '气运的轨迹变幻莫测，或许有更深厚的福缘等待',
                '魔法共鸣': '与魔法的共鸣可以更加深邃，元素的奥秘无穷',
                '战斗直觉': '直觉的敏锐可以在实战中不断 sharpen，越战越强',
                '治愈传承': '医者的传承博大精深，更高的医术境界尚待探索'
            };
            return hints[name] || '天赋有精进空间，稳步成长中';
        }
        
        // 普通级：基础天赋，但仍有进化可能
        const hints = {
            '生命强化': '生命力的洪流才刚刚涌动，体魄的极限远未触及',
            '魔力强化': '魔力之泉可以更加充沛，法力的上限仍可突破',
            '攻击强化': '破坏力的刻痕可以更深，力量的巅峰尚在前方',
            '防御强化': '天生的护甲只是起点，铜墙铁壁的境界可以更进一步',
            '速度强化': '迅如雷电的天赋可以更加极致，瞬息千里并非空谈',
            '自然恢复': '大地之子的韧性可以更强，生生不息的奥秘尚待领悟'
        };
        return hints[name] || '天赋虽基础，但仍有蜕变的可能';
    },
