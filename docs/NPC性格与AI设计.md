...
### 11.3 存量问题统计（v0.98.13后）

47个有对话树的NPC中：
- **有真正剧情触发（L1）**：36个（v0.98.0-13改造31个添加start_quest + 原有5个start_battle/open_shop）
- **只有数值变化（L3）**：10个（张小侯/赵满延/书店老板/魔法协会会长/神秘法师/何雨等，主要是close/back流程控制）
- **完全无影响**：1个（白藏风6节点，明珠校区NPC，博城篇后）

**改造成果**：v0.98.0-13共14个版本，改造31个NPC，新增31个对话触发任务。博城篇所有有对话树的NPC（除白藏风-明珠校区）均达到L1有意义对话标准。
