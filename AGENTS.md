...
> **文档原则**：每个文档都有明确的核心功能，不重复，不冗杂。找不到的内容先看AGENTS.md索引。

### 2.1 入门与概览类（根目录）
| 文档 | 核心功能 | 阅读优先级 |
|------|---------|-----------|
| **AGENTS.md**（本文档） | 开发主入口，文档索引，开发规范，经验沉淀 | ⭐⭐⭐ 每次开发前必读 |
| **TODO.md** | 开发计划，待办列表，优先级，模块完成度 | ⭐⭐⭐ 每次开发前必看 |
| **CHANGELOG.md** | 版本更新日志，了解最近改动 | ⭐⭐ 有新版本时看 |
| `README.md` | 项目说明，游戏介绍，面向玩家和开发者 | ⭐ 首次接触时看 |
...
### 2.3 系统设计类（每个系统一份设计文档）
| 文档 | 核心功能 | 阅读优先级 |
|------|---------|-----------|
| `docs/天赋系统设计文档.md` | 天赋系统设计（天生天赋、灵种、魂种、升级） | ⭐⭐ 修改天赋系统时 |
| `docs/战斗系统优化设计.md` | 战斗系统设计与优化记录 | ⭐⭐ 修改战斗系统时 |
| `docs/时间系统重设计方案.md` | 时间系统设计（8时段制、熬夜惩罚、课程表） | ⭐⭐ 修改时间系统时 |
...
| `docs/大事件系统设计方案.md` | 大事件/剧情系统设计 | ⭐⭐ 修改大事件系统时 |

### 2.4 内容开发类（从小说转化为游戏内容）
| 文档 | 核心功能 | 阅读优先级 |
|------|---------|-----------|
| `docs/小说内容逐章拆解工作流.md` | **小说拆解方法论v2.0**（7维度拆解+系统机制识别+6步融合流程） | ⭐⭐⭐ 从小说提取内容前必读 |
| `docs/小说内容转化指南.md` | 小说内容转化为游戏数据的具体方法 | ⭐⭐ 转化内容时参考 |
| `docs/第一卷开发计划.md` | 第一卷（博城篇）的详细开发计划 | ⭐⭐ 开发第一卷内容时 |
| `docs/游戏开发路线图.md` | 长期开发路线图，篇章规划 | ⭐ 了解长期规划时 |
| `docs/游戏内容规划与设计文档.md` | 整体内容规划与设计思路 | ⭐ 了解整体设计方向时 |

### 2.5 测试与调试类
| 文档 | 核心功能 | 阅读优先级 |
|------|---------|-----------|
| `docs/测试用例与发布检查清单.md` | **完整测试用例 + 发布前检查清单 + Bug模式总结** | ⭐⭐⭐ 每次发布前必读 |
| `docs/测试存档体系与最小单元化测试指南.md` | **测试存档体系 + 最小单元化黑盒测试方法** | ⭐⭐⭐ 安排测试任务前必读 |
| `docs/多Agent并行黑盒测试方法论.md` | 多Agent测试方法论，分段接力测试 | ⭐⭐ 安排测试任务时参考 |
| `docs/开发踩坑记录与通用经验.md` | 踩坑经验总结，避免重复踩坑 | ⭐⭐ 遇到问题时先看这个 |
| `docs/部署经验总结.md` | GitHub Pages部署经验 | ⭐ 部署时参考 |
| `docs/测试反馈与开发进度跟踪.md` | 历史Bug跟踪记录（档案） | ⭐ 了解历史bug时 |
...
### 2.7 文档阅读优先级总结
- **P0（必读）**：AGENTS.md → TODO.md → 数据格式规范.md → 小说内容逐章拆解工作流.md
- **P1（常用）**：内容扩展指南.md → 架构说明.md → 测试用例与发布检查清单.md → 测试存档体系指南.md
- **P2（按需）**：各系统设计文档 → 内容开发文档
- **P3（参考）**：README.md → CHANGELOG.md → 攻略文档 → 历史档案
...
| save_07_daily_test | Lv.3 | 日常系统专项测试 |
...
| `player.js` | 玩家系统 | SKILL_UNLOCK_TABLE, gainExp, levelUp, checkSkillUnlocks, awakenElement, talents |
| `talent.js` | 天赋系统 | initTalentForElement, getRandomTalent, getTalentEffects, addTalentExp |
| `battle.js` | 战斗系统 | startBattle, castSkill, enemyTurn, checkBattleEnd, calculateDamage |
...
| `daily.js` | 日常系统 | 每日签到，每日任务 |
| `big-event.js` | 大事件系统 | 剧情大事件，事件链 |
...
| `talents.js` | 天赋数据（10系×5个天赋，共50个） | 50项 |
