# 全职法师网页游戏 - 项目入口

> **这是唯一入口。** 读这一份就知道：项目是什么、方向在哪、怎么做、遇到问题看哪个文档。
> 具体任务看 [TODO.md](TODO.md)，版本历史看 [CHANGELOG.md](CHANGELOG.md)，详细设计看 [docs/](docs/)。
>
> 当前版本：**v3.20.0** | 最后更新：2026-09-10

---

## 一、项目定位

基于小说《全职法师》的网页游戏。**主角是玩家自己**，莫凡是背景中实力强劲的NPC。
玩家通过选择创造新剧情可能性，不脱离原著但不被原著束缚。轻松探索，人设不可破坏。

### 核心原则

1. **玩家有自己的人生**——不对标莫凡，不零和竞争，不截取他的机缘
2. **一切自然发生**——不刻意提示"XX不再关注YY"，不搞玩家主动操纵NPC关系
3. **玩家有真实影响力**——通过高密度对话分支、NPC记忆、信息差实现
4. **时间是叙事刻度**——不是稀缺资源，玩家想做什么就做什么
5. **人设不可破坏**——每个NPC互动符合其人设
6. **小说驱动，数据驱动**——不自己编设定，能用数据配置的不改代码
7. **设计先行，每步留痕**——新机制先写文档，每步git commit + 文档同步
8. **设计先对齐再实施**——涉及UI布局、系统机制、体验方向的设计，先把方案展示给用户确认，完全对齐后再开始写代码，不自行决定就实施
9. **标准流程：设计→对齐→实施→测试→回归**——每个新功能/大改动都严格按此流程：先出设计文档→用户确认对齐→写代码实施→黑盒测试→修复bug并回归验证，缺一步都不算完成

---

## 二、开发流程

```
1. 读 AGENTS.md 确认方向和当前版本
2. 看 TODO.md 找到当前任务
3. ⚠️ **涉及小说内容必须先读原文验证**：`全职法师.txt`（项目根目录，22.85MB完整原文），用Grep搜索章节/人物/设定，不凭记忆或已有拆解编造设定
4. 新机制 → 先写 docs/ 下的设计文档
5. 开发/改数据
   - ⚠️ **新增数据前必须先检索**：妖魔/技能/物品/NPC/事件等，先Grep确认ID和名称不重复，引用的技能/物品必须已存在
5. node --check 语法检查 + 无浏览器自动化测试
   - `node tests/run.js` 跑L1数据完整性+L2战斗逻辑（0.1秒出结果）
   - 改数据后必跑L1，改战斗逻辑后必跑L2
   - 详见 [测试工具文档](docs/测试存档体系与最小单元化测试指南.md)
6. 冒烟测试（能进游戏/能修炼/能对话）
7. 确认debug版本功能正常（新功能在debug中可快速触发）
8. 真黑盒测试（独立测试者，不知道代码，用debug快速进入对应阶段）
   - 功能测试：按[测试用例](docs/测试用例与发布检查清单.md)逐项验证
   - 体验测试：按体验测试用例评估节奏/压力/自由度/代入感
   - 只有L1-L3自动化覆盖不了的体验/视觉问题才需要浏览器测试
9. 修bug + 修体验问题
10. 文档同步（必做，不能跳过）
    - **基础同步**：TODO.md / CHANGELOG.md / AGENTS.md版本号 / ui.js标题
    - ⚠️ **README.md必须同步更新**：README是项目对外介绍，数据统计（任务/事件/妖魔/技能数量）、游戏特色、新功能介绍都要及时更新
    - **按开发动作更新对应文档**（见下方检查清单）
    - **轻量文档审计**（每次发布必做，5分钟）：检查本次涉及的方法论文档是否更新、AGENTS.md导航是否有失效链接、TODO.md进度是否更新
    - 详见 [docs/README.md](docs/README.md) 文档维护规则

**文档更新检查清单**（开发动作 → 必须更新的文档）：
| 开发动作 | 必须更新 |
|---------|---------|
| 新增数据字段/格式变化 | docs/数据格式规范.md |
| 新增系统/机制 | 对应核心设计文档 + docs/version-design/新版本设计 |
| 测试方法改进 | docs/测试方法论.md + docs/测试存档体系与最小单元化测试指南.md |
| 新增功能测试点 | docs/测试用例与发布检查清单.md |
| 遇到新坑并解决 | docs/开发踩坑记录与通用经验.md |
| 部署流程变化 | docs/部署经验总结.md |
| NPC对话/人设规则变化 | docs/NPC性格与AI设计.md |
| 篇章扩展流程改进 | docs/篇章扩展标准流程.md |
11. git commit + push（Gitee+GitHub）
```

**测试核心原则**：
- **分层测试**：L1数据/L2逻辑用自动化脚本（0.1秒），L3流程用jsdom，L4体验才用浏览器
- 真黑盒：浏览器测试时测试者不知道代码逻辑，纯玩家视角
- 体验优先：功能正常 ≠ 好玩，体验问题和bug同等重要
- debug先行：用debug快速推进，不要从Lv.1手动推到中期
- 详见 [测试方法论](docs/测试方法论.md) 和 [测试工具文档](docs/测试存档体系与最小单元化测试指南.md)

**Git路径**：`D:\Git\cmd\git.exe`（不在PATH，必须完整路径）
**启动游戏**：
- **直接打开**：双击根目录 `index.html` 即可运行（已包含构建产物，无需服务器）
- **开发模式**：`npm run dev`，访问 http://localhost:5173（支持热更新，修改代码后自动刷新）
- **重新构建**：修改代码后运行 `npm run build:copy`，会自动构建并复制到根目录，然后双击index.html即可运行
- **一键构建并运行**：`npm run play` 或双击 `build-and-play.bat`

**部署到GitHub Pages**：
- **分支模式**：master开发分支 + gh-pages部署分支（GitHub Pages）
- **自动化部署脚本**（推荐）：`.\tools\deploy.ps1 -Version "v3.2.0" -Message "版本描述"`
- **部署地址**：https://wsjshy.github.io/quanzhi-fashi-game/
- **详细部署流程和踩坑经验**：见 [docs/部署经验总结.md](docs/部署经验总结.md)
- ⚠️ **部署前必须确保master分支工作区干净**，否则切换分支会失败

---

---

## 一点五、提交前强制检查清单（不可跳过，缺一项不允许commit）

> **为什么有这个清单**：历史教训证明，描述性的规范会被选择性忽略。本清单是硬卡点，每次提交前必须逐项打勾，不允许跳过。隐瞒未执行项视为失败。

### 每次提交前必须输出并确认：

```
【vX.Y.Z 提交前强制检查清单】
□ 1. 语法检查：node --check 所有修改的JS文件（__/__通过）
□ 2. 自动化测试：node tests/run.js（__通过/__失败/__警告）
□ 3. 冒烟测试：关键路径模拟（__/__项通过）
   - 战斗启动/技能释放/状态效果/道具使用/战斗结束
□ 4. Debug验证：新功能在Debug中可快速触发（是/否，说明：__）
□ 5. 功能测试：黑盒用例（__/__项通过，列出关键用例）
□ 6. 体验测试：节奏/压力/自由度/代入感评估（__项良好/__项待优化）
□ 7. 文档同步：TODO/CHANGELOG/AGENTS/README（已更新__个/未更新__个）
□ 8. 数据唯一性：新增内容ID不重复（已检查/未检查）

以上8项全部打勾，才允许git commit。
未打勾的项必须说明原因和后续计划。
```

### commit message 必须包含测试结果摘要：

```
vX.Y.Z: 功能描述
测试：自动化__通过 | 冒烟__项 | 功能__项 | 体验__项
文档：已同步__个
```

没有测试结果的commit无效。

### 设计文档必须包含"测试要点"章节：

每个版本设计文档必须有：
```
## 测试要点
- 功能测试用例：（列出至少5个）
- 体验测试维度：（列出至少3个）
- Debug支持：（新功能如何在Debug中快速测试）
- 边界情况：（列出至少3个）
```
没有测试要点的设计文档无效。

### 每次完成后必须报告"规范执行审计"：

在每次开发完成后的回复中，第一部分必须是：
```
【规范执行审计】
本次开发遵循的规范：
- ✅ 标准开发流程11步：执行了__步，跳过了__步（原因：__）
- ✅ 测试策略：自动化/冒烟/黑盒/体验，执行了__项
- ✅ 文档同步：__个文档已更新
- ⚠️ 未执行的规范项：__（原因：__，后续计划：__）
```


## 二点五、新agent学习路径

> 第一次接触本项目？按以下顺序阅读，30分钟即可上手开发。

### 入门必读（3个文档，15分钟）
1. **本文件（AGENTS.md）** — 项目定位、9大核心原则、11步开发流程、文档导航
2. **[docs/架构说明.md](docs/架构说明.md)** — 代码结构、数据层/引擎层划分、模块组织
3. **[docs/数据格式规范.md](docs/数据格式规范.md)** — 所有数据格式，加内容前必读

### 按任务类型快速路径（选读）
| 你要做什么 | 读哪3个文档 |
|-----------|------------|
| 加妖魔/技能/物品 | 数据格式规范 → 对应data文件 → L1测试(`node tests/run.js`) |
| 加NPC对话 | [NPC性格与AI设计.md](docs/NPC性格与AI设计.md) → [对话树与剧情连锁系统设计.md](docs/对话树与剧情连锁系统设计.md) → characters.js |
| 加新系统 | 本文件开发流程 → 设计文档模板 → 架构说明 |
| 扩展小说篇章 | [篇章扩展标准流程.md](docs/篇章扩展标准流程.md) → [篇章级小说拆解方法论.md](docs/篇章级小说拆解方法论.md) → [小说内容转化指南.md](docs/小说内容转化指南.md) |
| 改UI | UIUX设计规范 → ui-assets.js → 对应ui-*.js模块 |
| 改战斗逻辑 | [战斗系统扩展与元素反应设计方法论.md](docs/战斗系统扩展与元素反应设计方法论.md) → battle子模块 → L2测试 |
| 修bug | [开发踩坑记录与通用经验.md](docs/开发踩坑记录与通用经验.md) → 测试方法论 → 对应模块 |

### 深入按需（不强制）
- 各系统设计文档（天赋/战斗/大事件/剧情等）
- 测试用例与发布检查清单
- 部署经验总结
- 项目批判与审视方法论

---

## 二点六、自迭代机制（按版本数量触发）

> 详细方法论见 [docs/项目自迭代与创新方法论.md](docs/项目自迭代与创新方法论.md)

| 触发时机 | 动作 | 耗时 | 输出 |
|---------|------|------|------|
| 每个版本发布后 | 版本反思会 | 30分钟 | docs/version-reflection/vX.Y.Z_反思.md |
| 每5个版本后 | 项目健康度审计 | 1小时 | 文档/代码/方法论/创新四维审计报告 |
| 每10个版本后 | 方法论评审会（需用户确认） | 2小时 | 方法论保留/修改/废弃/新增决策 |

**核心原则**：问题早发现，方法论持续改进，创新有空间，玩家体验优先。

---

## 二点七、规则分层与创新通道

> 详细说明见 [docs/项目自迭代与创新方法论.md](docs/项目自迭代与创新方法论.md) 第三章

| 层级 | 内容 | 能否打破 |
|------|------|---------|
| **铁律** | 玩家有自己的人生/人设不可破坏/小说驱动/数据驱动/设计先对齐 | ❌ 不可违反 |
| **规范** | 11步流程/文档同步/测试/代码规范/UI规范 | ⚠️ 小创新可走快速通道 |
| **建议** | 命名习惯/文档格式/具体实现方式 | ✅ 自由取舍 |

**快速创新通道**（小事件/小对话/小机制）：快速原型→自己玩觉得好玩→跑测试→补3-5行说明→提交。不需要完整设计文档和用户对齐（不涉及核心系统/UI大改）。

---

## 三、文档导航

> 📖 [docs/README.md](docs/README.md) — 文档总览，含目录结构和维护规则

### 任务与版本
| 想看什么 | 看哪个 |
|---------|--------|
| 当前做什么、任务清单、版本路线 | [TODO.md](TODO.md) |
| 版本变更历史 | [CHANGELOG.md](CHANGELOG.md) |

**版本号规则（语义化版本 vX.Y.Z）**：
- **X 主版本号**：重大架构重构、不兼容改动、游戏大阶段完成（如博城灾难完结→1.0）
- **Y 次版本号**：新增功能/新系统/新机制（向下兼容），如v0.77装备继承系统
- **Z 修订号**：纯Bug修复、小调整，不新增功能，如v0.77.1修复继承bug
- 既有新功能又修bug → Y+1，bug修复算在新版本里

### 核心设计文档（是什么）
| 主题 | 文档 |
|------|------|
| **项目存量内容清单（开发前必查）** | [docs/项目存量内容清单.md](docs/项目存量内容清单.md) |
| **博城篇原文回归校准报告** | [docs/博城篇原文回归校准报告.md](docs/博城篇原文回归校准报告.md) |
| 玩家影响力/分支叙事/NPC社交模拟 | [docs/玩家影响力系统设计.md](docs/玩家影响力系统设计.md) |
| 玩家专属机缘线 | [docs/玩家专属机缘线设计.md](docs/玩家专属机缘线设计.md) |
| 玩家个人任务线 | [docs/玩家个人任务线设计.md](docs/玩家个人任务线设计.md) |
| 游戏节奏/减负设计 | [docs/减负设计方案.md](docs/减负设计方案.md) |
| 剧情系统/章节/伏笔/分支 | [docs/剧情系统架构.md](docs/剧情系统架构.md) |
| 对话树引擎 | [docs/对话树与剧情连锁系统设计.md](docs/对话树与剧情连锁系统设计.md) |
| NPC关系/世界状态 | [docs/世界状态与关系网系统设计.md](docs/世界状态与关系网系统设计.md) |
| 大事件系统（博城灾难等） | [docs/大事件系统设计方案.md](docs/大事件系统设计方案.md) |
| 天赋/进化/效果系统 | [docs/天赋系统设计文档.md](docs/天赋系统设计文档.md) |
| 天赋设计方法论（怎么设计天赋） | [docs/天赋设计方法论.md](docs/天赋设计方法论.md) |
| **天赋系统差异化设计方法论（可扩展/新增系别流程）** | [docs/天赋系统差异化设计方法论.md](docs/天赋系统差异化设计方法论.md) |
| NPC成长系统（剧情阶段驱动） | [docs/NPC成长系统设计文档.md](docs/NPC成长系统设计文档.md) |
| 实力体系/数值公式 | [docs/实力体系设计文档.md](docs/实力体系设计文档.md) |
| 装备词缀系统 | [docs/装备词缀系统设计文档.md](docs/装备词缀系统设计文档.md) |
| 锻造系统（v3.9+v3.10深化） | [docs/锻造系统设计文档.md](docs/锻造系统设计文档.md) |
| 闭关修炼系统（v3.11） | [docs/version-design/v3.11.0_闭关修炼系统设计.md](docs/version-design/v3.11.0_闭关修炼系统设计.md) |
| 猎魔悬赏系统（v3.12） | [docs/猎魔悬赏系统设计文档.md](docs/猎魔悬赏系统设计文档.md) |
| 年度魔法考核事件（v3.13） | [docs/version-design/v3.13.0_年度魔法考核事件设计.md](docs/version-design/v3.13.0_年度魔法考核事件设计.md) |
| 项目整体架构 | [docs/架构说明.md](docs/架构说明.md) |

### 方法论文档（怎么做）
| 任务 | 看哪个 |
|------|--------|
| **篇章扩展标准流程（以博城篇为模板）** | [docs/篇章扩展标准流程.md](docs/篇章扩展标准流程.md) |
| **篇章级小说拆解方法论（怎么从篇章高度整合+设计转化）** | [docs/篇章级小说拆解方法论.md](docs/篇章级小说拆解方法论.md) |
| 拆小说转游戏内容 | [docs/小说内容逐章拆解工作流.md](docs/小说内容逐章拆解工作流.md) |
| 小说设定转游戏机制 | [docs/小说内容转化指南.md](docs/小说内容转化指南.md) |
| 加妖魔/技能/物品数据 | [docs/数据格式规范.md](docs/数据格式规范.md) |
| 加NPC/地点/事件 | [docs/非战斗系统可扩展性设计.md](docs/非战斗系统可扩展性设计.md) |
| NPC对话和行为设计 | [docs/NPC性格与AI设计.md](docs/NPC性格与AI设计.md) |
| 测试方法论（怎么测） | [docs/测试方法论.md](docs/测试方法论.md) |
| 测试用例（测什么） | [docs/测试用例与发布检查清单.md](docs/测试用例与发布检查清单.md) |
| 测试工具（用什么测） | [docs/测试存档体系与最小单元化测试指南.md](docs/测试存档体系与最小单元化测试指南.md) |
| 开发踩坑/经验 | [docs/开发踩坑记录与通用经验.md](docs/开发踩坑记录与通用经验.md) |
| 项目批判与审视方法论 | [docs/项目批判与审视方法论.md](docs/项目批判与审视方法论.md) |
| 部署经验 | [docs/部署经验总结.md](docs/部署经验总结.md) |
| **战斗系统扩展与元素反应设计方法论** | [docs/战斗系统扩展与元素反应设计方法论.md](docs/战斗系统扩展与元素反应设计方法论.md) |
| 新系统设计文档模板 | [docs/设计文档模板.md](docs/设计文档模板.md) |
| 构建与开发环境 | [docs/构建与开发环境指南.md](docs/构建与开发环境指南.md) |
| 存档系统与状态管理 | [docs/存档系统与状态管理指南.md](docs/存档系统与状态管理指南.md) |
| UI/UX设计规范 | [docs/UIUX设计规范.md](docs/UIUX设计规范.md) |
| 项目自迭代与创新方法论 | [docs/项目自迭代与创新方法论.md](docs/项目自迭代与创新方法论.md) |

### 文档目录
| 目录 | 内容 |
|------|------|
| [docs/version-design/](docs/version-design/) | 各版本设计文档（v0.48.0_*.md等，76个） |
| [docs/test-reports/](docs/test-reports/) | 黑盒测试报告、回归测试报告（27个） |
| [docs/reference/](docs/reference/) | 攻略、规划、状态记录等参考资料（14个） |
| [docs/小说拆解/](docs/小说拆解/) | 小说逐章拆解内容（按篇章重组：博城篇+明珠篇） |
| [docs/archive/](docs/archive/) | 废弃文档（保留历史） |

---

## 四、核心文件速查

### 小说原文
| 文件 | 作用 |
|------|------|
| 全职法师.txt | **小说原文**（项目根目录，22.85MB），涉及小说内容必须先Grep搜索验证，不凭记忆编造 |

### 数据文件（v3.0.0起迁移到src/data/，共25个模块）
| 文件 | 作用 |
|------|------|
| src/data/index.js | 数据统一出口（GameData + 27个全局变量） |
| src/data/skills.js | 技能数据（222个，11系初阶/中阶/高阶） |
| src/data/enemies.js | 妖魔数据（75个，奴仆/战将/统领级） |
| src/data/items.js | 物品数据（161个，消耗品/装备/材料/魔具） |
| src/data/characters.js | NPC对话/关系（49个有对话树） |
| src/data/quests.js | 任务数据（182个，主线/支线/日常/事件） |
| src/data/locations.js | 地点/动作（13个地点） |
| src/data/events.js | 随机事件（239个，探索/修炼/猎魔/社交） |
| src/data/big-events.js | 大事件（9个，博城灾难等） |
| src/data/talents.js | 系别天赋（55个，6种机制类型） |
| src/data/innate-talents.js | 天生天赋（27个，5种分类） |
| src/data/spirit-seeds.js | 灵种/魂种（46个） |
| src/data/star-dust-artifacts.js | 星尘魔器 |
| src/data/summon-beasts.js | 召唤兽及进化 |
| src/data/achievements.js | 成就（44个） |
| src/data/chapters.js | 章节数据 |
| src/data/shops.js | 商店数据 |
| src/data/maps.js | 地图数据 |
| src/data/clues.js | 线索数据 |
| src/data/demon-traits.js | 妖魔特性（机制型） |
| src/data/world.js | 势力/情报（8个势力） |
| src/data/affixes.js | 装备词缀配置（6前缀+6后缀+4档品质） |
| src/data/forge-recipes.js | 锻造配方（9个配方，基础/中级/高级） |
| src/data/bounties.js | 猎魔悬赏配置（11个任务，奴仆/战将/统领） |
| src/data/annual-exam.js | 年度魔法考核配置（策略/评分/奖励/对话） |

### 引擎文件（v3.0.0起迁移到src/engine/，v3.0.1核心文件拆分，共111个模块）
| 文件 | 作用 |
|------|------|
| src/engine/game.js | 游戏主流程（已拆分13个子模块，约3100行） |
| src/engine/battle.js | 战斗核心（已拆分23个子模块，约1850行） |
| src/engine/ui.js | UI渲染（已拆分25个子模块，约1400行） |
| src/engine/player.js | 玩家数据/成长 |
| src/engine/game-state.js | 集中式状态管理与存档（v3.0.2，声明式schema+版本迁移链） |
| src/engine/npc-state.js | NPC状态/记忆/关系网络 |
| src/engine/dialogue-tree.js | 对话树引擎 |
| src/engine/inventory.js | 背包/装备系统 |
| src/engine/forge.js | 锻造系统（v3.9） |
| src/engine/retreat.js | 闭关修炼系统（v3.11） |
| src/engine/bounty.js | 猎魔悬赏系统（v3.12） |
| src/engine/annual-exam.js | 年度考核系统（v3.13） |
| **UI子模块** | ui-title/ui-help/ui-bestiary/ui-daily/ui-dialogue/ui-event/ui-achievement/ui-character/ui-map/ui-battle/ui-shop/ui-talent-detail/ui-inventory/ui-message/ui-intel/ui-skill-detail/ui-quest/ui-enemy-detail/ui-scheduled-event/ui-damage-number/ui-element-selection/ui-reputation/ui-character-create/ui-primary-secondary/ui-daily-summary |
| **战斗子模块** | battle-utils/battle-start/battle-skill/battle-enemy-turn/battle-player-attack/battle-damage/battle-end-enemy-turn/battle-summon/battle-status/battle-rewards/battle-help/battle-magic-tool/battle-status-modifiers/battle-traits/battle-add-status/battle-check-end/battle-apply-status/battle-spirit-seed/battle-talent-active/battle-ai-burst/battle-end/battle-ai-tactical/battle-player-item |
| **游戏子模块** | game-end-battle/game-dialogue/game-npc-list/game-npc-detail/game-travel/game-awaken/game-breakthrough/game-cultivate/game-talent-select/game-quick-heal/game-artifact-upgrade/game-perform-cultivate/game-perform-action |
| src/main.js | 模块化入口（import 70+个模块） |

> 📖 详细架构见 [docs/架构说明.md](docs/架构说明.md)，拆分设计见 [docs/version-design/v3.0.1_核心文件拆分设计.md](docs/version-design/v3.0.1_核心文件拆分设计.md)

### 测试文件
| 文件 | 作用 |
|------|------|
| tests/run.js | 统一测试运行入口（`node tests/run.js`） |
| tests/utils.js | 测试工具（数据加载/断言/报告） |
| tests/unit/data-integrity.js | L1数据完整性测试 |
| tests/unit/battle-logic.js | L2战斗逻辑单元测试 |
| tests/unit/progression.js | L3成长流程测试 |

### 归档与备份（archive/）
| 目录/文件 | 内容 | 说明 |
|-----------|------|------|
| archive/engine-legacy/ | v3.0.0前的旧引擎文件（58个） | 旧版58个script标签架构的完整备份，用于回滚参考 |
| archive/index.legacy.html | 旧版入口HTML | 旧版58个script标签入口，需配合archive/engine-legacy/使用 |
| archive/tools-legacy/ | 已完成使命的工具脚本 | split-game-data.js、verify-split.js等一次性迁移工具 |

> ⚠️ archive/下的文件仅作备份参考，**不要在生产代码中引用**。当前活跃代码全部在src/下。

---

## 五、人设检查清单

每个NPC互动选项必须通过：
- [ ] 符合性格设定（高冷的不主动热情，傲慢的不低声下气）
- [ ] 符合身份（学生/老师/家族成员）
- [ ] 关系提升有合理事件支撑，不是凭空刷好感
- [ ] 无恋爱导向（除非人设+用户明确要求）
- [ ] 互动结果对剧情有实际影响
- [ ] **不围绕莫凡**——玩家有自己的人生

---

## 六、文档架构规范

> 所有文档按此分层，职责不重叠。新增文档时先判断属于哪一层。

### 分层原则

| 层级 | 文档 | 职责 | 不应该包含 |
|------|------|------|-----------|
| **入口层** | AGENTS.md | 项目定位、核心原则、开发流程、文档导航、快速参考 | 具体任务、版本历史、详细设计、常见坑 |
| **任务层** | TODO.md | 当前任务、版本规划、进度追踪 | 设计细节、方法论、版本历史 |
| **历史层** | CHANGELOG.md | 版本变更记录 | 未来规划、设计细节 |
| **设计层** | docs/*.md | 各系统详细设计、方法论 | 任务清单、版本历史 |
| **归档层** | docs/archive/ | 废弃文档 | 活跃内容 |

### 写作规则

1. **AGENTS.md控制在~100行**——只放方向和导航，细节指向其他文档
2. **TODO.md只放任务**——每个版本一个章节，状态用[x]/[ ]/[~]标记
3. **新系统必须先写docs/设计文档**，再写代码
4. **废弃文档移到docs/archive/**，不删除（保留历史）
5. **重复文档合并**——同一主题只保留一份最新的
6. **AGENTS.md的文档导航必须覆盖所有活跃文档**——确保从入口能找到一切
