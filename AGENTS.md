...
> **背景**：v3.21.0开发中发现，内容溯源原则只覆盖了"新增内容"，没有覆盖"裁剪/修改/重构"，导致把原著核心内容（暴躁之泉/宇昂）错误识别为非原著内容。本原则扩展所有内容操作的溯源要求。

### 所有内容操作必须回原文验证

| 操作类型 | 是否需要回原文验证 | 验证内容 |
|---------|-------------------|---------|
| 新增内容 | ✅ 必须 | 是否符合原著设定，是否与现有内容重复 |
| 裁剪/删除内容 | ✅ 必须 | 是否是原著核心内容（核心内容不可裁剪） |
| 修改现有内容 | ✅ 必须 | 是否违背原著设定 |
| 重构现有系统 | ✅ 必须 | 核心设定是否是原著，是否保留 |
| 纯bug修复 | ⚠️ 可选 | 不涉及设定变更时可跳过 |
| 纯UI优化 | ⚠️ 可选 | 不涉及内容变更时可跳过 |

### 原著内容数据库（强制参考）

- **博城篇**：`docs/博城篇原著内容数据库.md`（核心机制/重要角色/关键物品/场景地点）
- **后续篇章**：扩展时建立对应数据库

**使用规则**：
1. 新增内容前：查询数据库，确认是否已有类似原著内容
2. 裁剪内容前：查询数据库，确认是否是原著核心内容
3. 修改内容前：查询数据库，确认是否违背原著设定
4. 发现数据库缺失：立即回原文验证，并补充到数据库
5. 每次涉及原著内容的开发后：必须更新数据库
...
## 原著验证清单
本次设计涉及的内容操作：
- [ ] 新增内容：（列出，已回原文验证：是/否，出处：第X章）
- [ ] 裁剪内容：（列出，已回原文验证：是/否，验证结果：原著/原创/改编）
- [ ] 修改内容：（列出，已回原文验证：是/否，验证结果：符合原著/违背原著）
- [ ] 重构内容：（列出，已回原文验证：是/否，核心设定：原著/原创）

所有涉及原著内容的操作，必须附原文出处（章节+关键引文）。
```
...
后续小说拆解不能只做脉络梳理，必须包含：
1. **剧情脉络**：发生了什么事
2. **核心机制**：有什么重要设定/系统/规则
3. **重要角色**：出场角色+角色定位+人物关系
4. **关键物品**：重要道具/魔具/资源
5. **场景地点**：重要地点+环境设定
6. **可游戏化点**：哪些内容可以转化为游戏玩法
